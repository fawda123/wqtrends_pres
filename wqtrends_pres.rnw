...
<</predvals, echo = F, message = F, fig.width = 8.5, fig.height = 4, fig.cap = 'Predicted and observed monthly chlorophyll by segment.', out.width = '\\linewidth'>>=
...
\begin{columns}[T]
\begin{column}{0.5\textwidth}
<</segmap, fig.height = 4, fig.width = 4, echo = F, results = 'hold'>>=
...
title('Segment-based approach', line = 0, cex.main = 1.6)
@
\end{column}
\begin{column}{0.45\textwidth}
...
<</segsg, fig.height = 4, fig.width = 4, echo = F, results = 'hold'>>=
...
\begin{column}{0.32\textwidth}
Get depth points\\~\\
<</segpt, fig.height = 4, fig.width = 4, echo = F, results = 'hold'>>=
...
sp::plot(seg)
sp::plot(state, add = T, col = pal(5)[3])
points(sgrass, col = alpha(pal(5)[5], 0.5), pch =16, cex = 1.5)
@
\end{column}
\end{columns}
\onslide<+->
...
rsc.pts<-rescale(segs$ZcMax,c(1,7))
...
\onslide<+->
\begin{column}{0.45\textwidth}
<</docfail1, fig.height = 4, fig.width = 4, echo = F, results = 'hold'>>=
seg <- readShapeSpatial('data/seg_820.shp')
state <- readShapeSpatial('data/FL_state.shp')
sgrass <- readShapeSpatial('data/sgpts_820_2006.shp')
sgrass <- sgrass[sample(1:nrow(sgrass), 2000, replace = F),]
...
sp::plot(state, add = T, col = pal(5)[3])
sp::plot(seg, add = T)
points(above, col = alpha(pal(5)[5], 0.5), pch = 16)
points(below, col = alpha(pal(5)[5], 0.5), pch = 21)
legend('bottomleft', legend = c('Within max depth estimate', 'Beyond max depth estimate'), col = c(alpha(pal(5)[5], 0.5),alpha(pal(5)[5], 0.5)), pch = c(16, 21), border = white)
@
\end{column}
\onslide<+->
\begin{column}{0.45\textwidth}
<</docfail2, fig.height = 4, fig.width = 4, echo = F, results = 'hold'>>=
seg <- readShapeSpatial('data/seg_820.shp')
state <- readShapeSpatial('data/FL_state.shp')
sgpoly <- readShapeSpatial('data/bbend_sg_diss.shp')
sgrass <- readShapeSpatial('data/sgpts_820_2006.shp')
...
for(rad in rads){
  ...
  sp::plot(state, add = T, col = pal(5)[3])
  sp::plot(seg, add = T)
  points(buff_pts, pch = 16, col = alpha('red', 0.5))
...
rsc_ests <- rescale(rsc_ests, c(1, 5))
